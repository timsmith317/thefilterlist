...
// Largest allowed edge (px) for a persisted reference photo. The crop already
// bounds most photos, but a tight tall/wide crop kept at full capture resolution
// can still be several MB. Capping the LONGER edge (not forcing a square) keeps
// the tall label/model-number strips this cropper is designed for while bounding
// the worst case — no photo exceeds ~200-250 KB regardless of how it's cropped.
// This is what keeps cross-device photo sync cheap. 1440 (over 1024) is chosen
// to keep small text — part numbers — crisp; the storage cost is negligible.
// Only affects NEWLY added photos; existing stored photos are untouched.
